...
//            cookie("cookie_code",$post["cookie_code"],86400*15);
    ...
    public function bind_phone(){
        $id = input('param.','');
        $m = new LoginModel();
        $list = $m->get_use_list($id['member_id']);
...
//                    'redirect_uri' => 'http://live.mengdie.com/index.php/zb/user/zb_login_callback/plat/qq.html',
                ...
                $data = [
                    ...
                    "pic_path_name"=>$info["figureurl_qq_2"],
                    "longitude"=>"",
                    "latitude"=>""
                ];
                $m = new LoginModel();
        ...
        if($res['third'] == 1){
            ...
            $list = $rt->get_use_list($member_id);
            if(empty($list['phone'])){
                $this->redirect(url('index/login/bind_phone',['member_id' =>$member_id]));
            }else{
