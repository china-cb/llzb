...
//				$this->res_headerstr = $ar_back['0'];
	...
	private function __optSets(){
		...
		// default:GET
		$options = array(
				CURLOPT_HEADER => false,//$this->_return_header,  // true:显示头信息,false不显示头信息
	...
	public function post($url,$forms=array()){
		...
		if(false !== $response){
			$ar_back= explode(PHP_EOL.PHP_EOL, $response,2);
			if($ar_back){
				$body = $this->res_headerstr = $ar_back['0'];
//				$body = $ar_back['1'];
//
				$this->parse_response_header($this->res_headerstr);
			}
	...
	/**
	 *带wablive的特殊请求post 加入CURLOPT_USERAGENT
	 */
	public function live_post($url,$forms=array()){
		return $this->sendRequest($url, $paramters = $forms, $method = 'POST', $needAccess = 0);
	}

	private function sendRequest($url, $paramters = array(), $method = 'POST', $needAccess = 0)
	{

		$curl = curl_init();
		$requesetUrl = $url;
		if (!empty($paramters) && ($method == 'GET'))
			$requesetUrl .= '?' . http_build_query($paramters);
		curl_setopt($curl, CURLOPT_URL, $requesetUrl);

		$header = array();
		if (!empty($paramters) && ($method != 'GET'))
			curl_setopt($curl, CURLOPT_POST, 1);
		if (is_string($paramters)) {
			curl_setopt($curl, CURLOPT_POSTFIELDS, ($paramters));//json
		} else {
			curl_setopt($curl, CURLOPT_POSTFIELDS, http_build_query($paramters));//对象或者数组
		}

		curl_setopt($curl, CURLOPT_HTTPHEADER, $header);
		curl_setopt($curl, CURLOPT_USERAGENT, 'Mozilla/4.0 (compatible; MSIE 8.0; Windows NT 6.0; Trident/4.0) webLive.0');
		...
		if ($result === false) {
			//        recordlog(curl_error($curl));
			echo 'shibai';

		}
		curl_close($curl);

		if (ord( $result[0] ) == 239 && ord( $result[1] ) == 187&& ord( $result[2] ) == 191 ) {//Bom头是固定的，可以检测后去除掉
