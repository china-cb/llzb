...
class QC extends Oauth{
     ...
     * @return Object QC
     */
    public function __construct($access_token = "", $openid = "", $appid = ""){
        parent::__construct();
        ...
        if($access_token === "" || $openid === ""){
            ...
            $this->keysArr = array(
                "oauth_consumer_key" => !empty($appid)?(int)$appid:(int)$this->recorder->readInc('appid'),
  ...
  private function get_user_info(){
      $api_user=$this->APIMap;
      $params=$this->kesArr;
      $url=$api_user['get_user_info'][0].'?oauth_consumer_key='.$params['oauth_consumer_key'].'&access_token='.$params['access_token'].'&openid='.$params['openid'].'&format=json';
      return json_decode($this->urlUtils->get_contents($url),true);
  }
    //调用相应api
