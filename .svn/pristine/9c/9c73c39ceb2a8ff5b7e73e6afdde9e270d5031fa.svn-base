...
class ChannelSetModel extends Model
{
    ...
    public function get_user_av_room_list($id,$mid){
        return db_func('user_av_room','sp_')
            ->where(['id' => $id,'uid' => $mid])
            ->field('cover,reward_data,phone_data,secret_key,unit_price,map_guide,guide_type,type,window,logo,first_logo,logo_type,logo_position,reward_switch,phone_switch')
            ->find();
    }
    /** 直播间设置 */
