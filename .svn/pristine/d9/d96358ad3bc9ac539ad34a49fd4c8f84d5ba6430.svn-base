...
class T163 extends Driver
{
     ...
     * @var string
     */
    protected $getRequestCodeURL = 'https://api.t.xiangchang.com/oauth2/authorize';

    /**
     * 获取access_token的api接口
     * @var string
     */
    protected $getAccessTokenURL = 'https://api.t.xiangchang.com/oauth2/access_token';

    /**
     * API根路径
     * @var string
     */
    protected $apiBase = 'https://api.t.xiangchang.com/';
    ...
    protected function parseToken($result)
    {
        ...
        if ($data['uid'] && $data['access_token'] && $data['expires_in'] && $data['refresh_token']) {
            ...
            throw new \Exception("获取香肠微博ACCESS_TOKEN出错：{$data['error']}");
        }
