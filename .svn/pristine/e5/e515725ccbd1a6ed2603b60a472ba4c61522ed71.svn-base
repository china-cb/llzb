...
class WisApi
{
    ...
    public function WisList($param){
        if(!isset($param["skip"]) || !isset($param["num"])){
            return array(
                'Flag'=>101,
                'FlagString'=>'参数错误'
            );
        }
        $path = "/v1/wis?skip=".$param['skip']."&num=".$param['num'];
        if(isset($param['state'])){
            $path = $path."&state=".$param['state'];
        }
    ...
    public function PageInfo($param){
        if(empty($param['docId']) || empty($param['page']) || empty($param['wisId'])){
            return array(
                'Flag'=>101,
                'FlagString'=>'参数错误'
            );
        }
        return $this->curl("/v1/wis/docs/".$param['docId']."/pages/".$param['page']."?wisId=".$param['wisId']);
    }
    ...
    public function SyncPage($param){
        if(empty($param['wisId']) || empty($param['page'])  || empty($param['clientid'])){
            return array(
                'Flag'=>101,
                'FlagString'=>'参数错误'
            );
        }
        return $this->curl("/v1/wis/".$param['wisId']."/syncPage","POST",json_encode(array('page' =>  intval($param['page']),'clientid'=>$param['clientid'])));
    }
    ...
    //白板开启和关闭
    public function off_Wis($param){
        if(empty($param['wisId'] || !isset($param['state']))){
            return array(
                'Flag'=>101,
                'FlagString'=>'参数错误'
            );
        }
        return $this->curl("/v1/wis/".$param['wisId']."/op","POST",json_encode($param));
    }
