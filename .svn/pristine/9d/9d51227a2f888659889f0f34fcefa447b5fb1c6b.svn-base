...
Class Business
{
    ...
    public function get_push_url($stream){
        ...
        $time = NOW_TIME;
        $publish_k = md5($config['app_id'].$stream.$config['publish_token'].$time); //推流加密防盗链
        $play_k = md5($config['app_id'].$stream.$config['play_token'].$time); //推流加密防盗链
        $hls_k = md5($config['app_id'].$stream.$config['hls_token'].$time);  //推流加密防盗链

        $address = [
            'push_url'=>"rtmp://26836.lsspublish.aodianyun.com/".$config['app_id']."/".$stream."?k=".$publish_k."&t=".$time,
            'rtmp'=>"rtmp://26836.lssplay.aodianyun.com/".$config['app_id']."/".$stream."?k=".$play_k."&t=".$time,  //仅用于PC
            'hls'=>"http://26836.hlsplay.aodianyun.com/".$config['app_id']."/".$stream."?k=".$hls_k."&t=".$time,   //可同时用于PC和移动端
        ];
        return $address;
    }
...
//        dump($arr);
                ...
                if(empty($package_info) || ($package_info['balance'] < $cost_min) || ($package_info['expire_time'] < NOW_TIME)){
                    ...
                    if(!empty($new_package)){
                        ...
                        try {
                            ...
                            if($package_info['expire_time'] > NOW_TIME){ //说明套餐未过期
                                //将上一个套餐余额清零并设置为已用完状态
                                $res2 = db("user_package")->where('id',$package_info['id'])->where('balance','>',0)->update(['status'=>2,'balance'=>0]);
                                if($res2 <= 0) throw new \Exception($package_info['id']."号套餐余额清零失败");
                                $cost_mins = intval($length) - intval($package_info['balance']);  //新套餐应扣时长
                                $cost_money = ($package_info['unit_price']*$package_info['balance'] + $new_package['unit_price']*($cost_min));
                            }else{  //套餐已过期
                                $res2 = db("user_package")->where('id',$package_info['id'])->where('balance','>',0)->update(['status'=>-2]);
                                if($res2 <= 0) throw new \Exception($package_info['id']."号套餐过期状态设置失败");
                                $cost_mins = intval($length);  //新套餐应扣时长
                                $cost_money = $new_package['unit_price']*($cost_min);
                            }
                            $package_rest = intval($res2['balance'] - $cost_mins);
                            //生成消费订单信息
                            $data = [
                            ...
                            //扣除套餐余额
                            $result = db("user_package")->where("id",$new_package['id'])->setDec("balance",$cost_mins);
                            if($result <= 0) throw new \Exception($new_package['id']."号套餐消费时长扣除失败");
                            Db::commit(); //提交事务
                        } catch (Exception $e) {
                            log_sql($uid."的".$cid."号频道:".$e->getMessage(),-1);
                        ...
                        if($result['Flag'] != 100){
                            log_sql($uid."_".$cid."用户已欠费,停止发布失败,失败原因:".$result['FlagString'],-1);
                            log_w($uid."_".$cid."用户已欠费,停止发布失败,失败原因:".$result['FlagString']);
                        }
                        //同时将该直播记录结束
                        $res = $m->update_live_record($cid);
                        //将套餐余额清零
                        $res2 = db("user_package")->where(['id'=>$package_info['id'],'status'=>1])->update(['status'=>-2,'balance'=>0]);
                        //并将该套餐
                        if($res > 0 && $res2 > 0){
                            continue;
                        }else{
                            log_sql($uid."_".$cid."用户已欠费,但未将live_record杀死",-1);
                    ...
                    try {
                        ...
                        $data = [
                            ...
                            "package_rest"=>($package_info['balance'] - $cost_min),
                            "status"=>1,
                            "create_time"=>NOW_TIME
                        ];
                        $res = db("cost_orders")->insert($data);
    ...
    public function get_first_url(){
        ...
        if(!empty($live_record) && ($live_record['status'] == 1)){
...
//                dump($result);
        ...
        if($live_record['status'] == -1){
            wrong_return("直播已结束,请观看其它直播");
        }
        ok_return("该频道未切换流", 2);
    }
        ...
        if(!empty($list)){
            ...
            foreach($list as $k=>$v){
                ...
                if($result['Flag'] == 100 && $result['Living'] == 0){ //说明该频道不在直播了,更新直播记录
                    $res = $m->update_live_record($v['cid']);
                    if($res <= 0 ){
                        log_w("id为".$v['id']."的直播记录杀死失败!!!");
                        log_sql("id为".$v['id']."的直播记录杀死失败!!!",-1);
                    }
                }
            };
        }
    ...
    public function check_account_balance(){
        $uid = input('param.uid','');
        empty($uid) && wrong_return("用户ID不存在");
        $m = new BusinessModel();
        $package_info = $m->get_user_package($uid);
        (empty($package_info)) && wrong_return("请先开通套餐再开始直播");
        ($package_info['expire_time'] < NOW_TIME) && wrong_return("套餐已过期,请先开通新的套餐");
        ($package_info['balance'] == 0) && wrong_return("您的套餐已使用完,请及时开通新的套餐,以免影响您的使用");
        ok_return("success");
    }
