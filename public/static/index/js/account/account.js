...
$(function () {
    ...
    var func = {
        init: function () {
            var user_type = $("#name_authentication").val();
        ...
        update_recharge:function(page){
           var sta_date = $("#start").html();
            var end_date = $("#end").html();
            var used_url=$('#recharge_ajx').val();
        $.post(used_url,{page:page,sta_date:sta_date,end_date:end_date}, function (data) {
            if(data.code==1){
        ...
        $.ajax({
            ...
            success : function(rt){
                if (rt.code == 1) {
                    ...
                    $(".positive_img").addClass('span_succes');
                }else{
        ...
        $.ajax({
            ...
            success : function(rt){
                if (rt.code == 1) {
                    ...
                    layer.msg(rt.msg,{icon:1});
                    $(".business_img").addClass('span_succes');
                }else{
                    layer.msg(rt.msg,{icon:2});
                }
            }
        ...
        $.ajax({
            ...
            success : function(rt){
                if (rt.code == 1) {
                    ...
                    layer.msg(rt.msg,{icon:1});
                    $(".negative_img").addClass('span_succes');
                }else{
                    layer.msg(rt.msg,{icon:2});
                }
            }
    ...
    $(document).on('change','#balance',function() {
        var re = /^[0-9]*[1-9][0-9]*$/ ;
        var rmb = $("#balance").val();
        if(isNaN(rmb)|| !re.test(rmb) || rmb<0){
    ...
    //自定义充值
    $(document).on('focus','#balance',function() {
        var rmb = $("#balance").val();
        $("#balance_all").val(rmb);
    });
    ...
    $(document).on('click','.at_recharges',function(){
        ...
        var re = /^[0-9]*[1-9][0-9]*$/ ;
        if(isNaN(money)|| !re.test(money) || money<0){
